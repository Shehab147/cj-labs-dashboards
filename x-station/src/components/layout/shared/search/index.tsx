...
const CommandFooter = ({ t }: { t: typeof enDict.search }) => {
  return (
    <div cmdk-footer=''>
...
const NavSearch = () => {
  ...
  const transformedData = useMemo(() => {
    return data.reduce((acc: Section[], item) => {
      const sectionTitle = nav[item.sectionKey as keyof typeof nav] || item.sectionKey
      const existingSection = acc.find(section => section.title === sectionTitle)

      const newItem = {
        id: item.id,
        name: nav[item.nameKey as keyof typeof nav] || item.nameKey,
        url: item.url,
        excludeLang: item.excludeLang,
        icon: item.icon,
        shortcut: item.shortcut
  ...
  const onSearchItemSelect = (item: Item) => {
    item.url.startsWith('http')
      ...
      : router.push(item.excludeLang ? item.url : getLocalizedUrl(item.url, currentLocale))
    setOpen(false)
  }
    ...
    <>
      {isBreakpointReached || settings.layout === 'horizontal' ? (
        ...
        <div className='flex items-center gap-2 cursor-pointer' onClick={() => setOpen(true)}>
          ...
          <div className='whitespace-nowrap select-none text-textDisabled'>{t.placeholder} ⌘K</div>
        </div>
      )}
      <CommandDialog open={open} onOpenChange={setOpen}>
        <div className='flex items-center justify-between border-be pli-4 plb-3 gap-2'>
          ...
          <CommandInput value={searchValue} onValueChange={setSearchValue} placeholder={t.placeholder} />
                  ...
                  {section.items.map((item, index) => {
                    return (
                      <SearchItem
                        ...
                        url={getLocalizedUrl(item.url, currentLocale)}
