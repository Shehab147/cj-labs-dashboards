...
import { productApi, categoryApi, menuApi, stockApi } from '@/services/api'
...
const ProductsManagement = () => {
  ...
  const [stockItems, setStockItems] = useState<Array<{ id: number; name: string; unit: string }>>([])
  ...
  const [formData, setFormData] = useState({ name: '', price: '', cost: '', category_id: '', description: '' })
  ...
  const handleOpenAdd = () => {
    setEditProduct(null)
    setFormData({ name: '', price: '', cost: '', category_id: categories[0]?.id?.toString() || '', description: '' })
    setDialogOpen(true)
  }
  ...
  const handleOpenEdit = (product: Product) => {
    ...
    setFormData({
      ...
      cost: String(product.cost_estimate || ''),
  ...
  const openIngredientsDialog = async (product: Product) => {
    try {
      setIngredientsLoading(true)
      setIngredientsProduct(product)
      setIngredientsDialogOpen(true)

      const [menuItemRes, stockRes] = await Promise.all([
        menuApi.getItem(String(product.id)),
        stockApi.getStock()
      ])

      if (stockRes.status === 'success') {
        const items = Array.isArray(stockRes.data?.items) ? stockRes.data.items : []

        setStockItems(items.map((item: any) => ({ id: item.id, name: item.name, unit: item.unit })))
      } else {
        setStockItems([])
      }

      if (menuItemRes.status === 'success') {
        const ingredients = menuItemRes.data?.item?.ingredients || []

        setIngredientsRows(
          ingredients.map((ing: any) => ({
            stock_item_id: String(ing.stock_item_id),
            qty_needed: String(ing.qty_needed ?? '')
          }))
        )
      } else {
        setIngredientsRows([])
      ...
      setStockItems([])
      setIngredientsRows([])
      setSnackbar({ open: true, message: 'فشل تحميل المكوّنات', severity: 'error' })
    } finally {
  ...
  const addIngredientRow = () => {
    setIngredientsRows(prev => [...prev, { stock_item_id: '', qty_needed: '' }])
  }

  const updateIngredientRow = (index: number, key: 'stock_item_id' | 'qty_needed', value: string) => {
    setIngredientsRows(prev => prev.map((row, i) => (i === index ? { ...row, [key]: value } : row)))
  }

  const removeIngredientRow = (index: number) => {
    setIngredientsRows(prev => prev.filter((_, i) => i !== index))
  }

  const handleSaveIngredients = async () => {
    if (!ingredientsProduct) return

    const cleaned = ingredientsRows
      .filter(row => row.stock_item_id && row.qty_needed !== '')
      .map(row => ({
        stock_item_id: Number(row.stock_item_id),
        qty_needed: Number(row.qty_needed)
      }))
      .filter(row => !Number.isNaN(row.stock_item_id) && !Number.isNaN(row.qty_needed))

    try {
      const res = await menuApi.setIngredients({
        menu_item_id: ingredientsProduct.id,
        ingredients: cleaned
      })

      if (res.status === 'success') {
        setSnackbar({ open: true, message: 'تم حفظ المكوّنات بنجاح', severity: 'success' })
        setIngredientsDialogOpen(false)
      } else {
        setSnackbar({ open: true, message: res.message || 'فشل حفظ المكوّنات', severity: 'error' })
      }
    } catch {
      setSnackbar({ open: true, message: 'فشل حفظ المكوّنات', severity: 'error' })
    }
  }
...
return (
                    <TableRow key={product.id}>
                      ...
                      <TableCell>{parseFloat(String(product.cost_estimate || 0)).toFixed(2)}</TableCell>
                      <TableCell>
                        <Chip
                          label={product.is_active ? (product.is_available ? 'متاح' : 'غير متاح') : 'معطل'}
                          size='small'
                          color={product.is_active ? (product.is_available ? 'success' : 'warning') : 'default'}
                        />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell align='right'>
                        <IconButton size='small' onClick={() => openIngredientsDialog(product)} color='secondary'>
      ...
      <Dialog open={ingredientsDialogOpen} onClose={() => setIngredientsDialogOpen(false)} maxWidth='md' fullWidth>
        <DialogTitle>إدارة مكوّنات الصنف: {ingredientsProduct?.name || ''}</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: '1rem !important' }}>
          {ingredientsLoading ? (
            <div className='flex items-center justify-center min-h-[120px]'><CircularProgress size={24} /></div>
          ) : (
            <>
              {ingredientsRows.map((row, index) => (
                <Box key={index} className='flex items-center gap-3'>
                  ...
                  <TextField
                    ...
                    type='number'
                    value={row.qty_needed}
                    onChange={e => updateIngredientRow(index, 'qty_needed', e.target.value)}
              ...
              <Button variant='outlined' startIcon={<i className='tabler-plus' />} onClick={addIngredientRow}>
                إضافة مكوّن
              </Button>
            </>
          )}
        ...
        <DialogActions>
          <Button onClick={() => setIngredientsDialogOpen(false)}>{t.common.cancel}</Button>
          <Button variant='contained' onClick={handleSaveIngredients} disabled={ingredientsLoading}>حفظ المكوّنات</Button>
        </DialogActions>
      </Dialog>
