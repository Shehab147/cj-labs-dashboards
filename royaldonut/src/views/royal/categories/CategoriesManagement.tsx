...
const CategoriesManagement = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 'success') {
        setSnackbar({ open: true, message: t.categories.categoryAdded, severity: 'success' })
        setDialogOpen(false)
        fetchCategories()
      } else {
    ...
    <>
      <Card>
        <CardHeader
          ...
          action={
            <Button variant='contained' startIcon={<i className='tabler-plus' />} onClick={() => { setName(''); setDialogOpen(true) }}>
