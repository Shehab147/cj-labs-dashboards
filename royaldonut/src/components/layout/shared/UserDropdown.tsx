...
return
        ...
        const response = await cashierShiftApi.getMy()

        if (response.status === 'success') {
          setHasActiveShift(Boolean(response.data?.shift))
        }
      } catch (error) {
  ...
  const userName = admin?.name || user?.full_name || session?.user?.name || ''
  const userEmail = user?.username || session?.user?.email || ''
  const userImage = session?.user?.image || ''
  const showShiftControls = useMemo(() => isCashier, [isCashier])
  ...
  const handleShiftConfirm = async () => {
    ...
    try {
      ...
      if (shiftDialogType === 'end') {
        const response = await cashierShiftApi.end({ closing_cash: cashValue })
        ...
        const response = await cashierShiftApi.start({ opening_cash: cashValue })
